...
const PullCurrentSemesters = () => {
  const [courseId, setCourseId] = useState("");
  const [status, setStatus] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState(null);
  const apiUrl = `https://knj.horus.edu.eg/api/hue/portal/v1/Current-semesters/${courseId}`;
  ...
  return (
    <div className="flex-wrap justify-center flex items-baseline ">
      <label className="block text-gray-700 mt-4  dark:text-white ">
        Current Semesters
        <input
          id="studentId"
          className="border border-black rounded-md p-4 mt-1 ml-4 dark:text-black dark:text-black"
          type="number"
          value={courseId}
...
export default PullCurrentSemesters;
