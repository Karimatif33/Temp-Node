...
const GetCatQuesTypeData = () => {
  const [status, setStatus] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState(null);
  const apiUrl = `https://knj.horus.edu.eg/api/hue/portal/v1/CatQuesTypeData`;
  ...
  const handlePullData = async () => {
    try {
      ...
      if(setIsLoading){
        ...
        toast.success('CatQuesType Loading...')
      }
...
export default GetCatQuesTypeData;
